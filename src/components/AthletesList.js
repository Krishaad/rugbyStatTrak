...
const AthleteList = () => {
  ...
  const fetchAthletes = async () => {
    try {
      const response = await fetch(
        "https://connect-eu.catapultsports.com/api/v6/athletes",
        {
          method: "GET",
          headers: {
  ...
  const handleButtonClick = (athlete) => {
  ...
  return (
    <div>
              ...
              <li key={athlete.id}>
                <button
                  style={{
                    ...styles.button,
                    backgroundColor:
                      athlete.id === selectedPlayer?.id ? "#add8e6" : "white",
                  }}
                  onClick={() => handleButtonClick(athlete)}
                >
      ...
      <div style={styles.rightContainer}>
        {selectedPlayer && (
          <PlayerDetails player={selectedPlayer} onClose={handleCloseModal} />
        )}
      </div>
    </div>
  );
